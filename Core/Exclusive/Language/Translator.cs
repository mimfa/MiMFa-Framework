...
namespace MiMFa_Framework.Exclusive.Language
{
    public class MiMFa_Translator
    {
        ...
        public virtual dynamic TryDone(dynamic obj)
        {
            ...
            try
            {
                if ((t is IDictionary || tn.StartsWith("dictionary") || tn.StartsWith("mimfa_dictionary"))) return Done(MiMFa_Convert.ToDictionary(obj));
                if (tn.StartsWith("keyvaluepair")) return Done(MiMFa_Convert.ToKeyValuePair(obj));
                if (t is IList || tn.StartsWith("list") || tn.StartsWith("mimfa_list")) return Done(MiMFa_Convert.ToList(obj));
                if (tn.EndsWith("[]")) return Done(MiMFa_Convert.ToArray(obj));
                if (t is ICollection) return Done(MiMFa_Convert.ToArray(obj));
        ...
        public virtual string Done(string arg)
        {
            ...
            return MiMFa_LanguageReader.GetText(SignTranslate + arg);
        }
