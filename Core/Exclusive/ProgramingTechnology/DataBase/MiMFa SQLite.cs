...
namespace MiMFa_Framework.Exclusive.ProgramingTechnology.DataBase
{
    [Serializable]
    public class MiMFa_SQLite : IProgramingTechnology
    {
        ...
        public string TempDirectory
        {
            ...
            set { MiMFa_Path.CreateAllDirectories(_TempDirectory = value); }
        }
        public Version Version => new Version(1, 0, 0, 0);

        #region Properties
        public MiMFa_SQLite_Types TYPE { get; set; } = new MiMFa_SQLite_Types();
        ...
        #region Main
        public MiMFa_SQLite(string address = null,string password = "", decimal version = 3, int timeout = 3000, bool uTF8Encoding = true, string tableName = null, params string[] columnsname)
        {
        ...
        public  void Start(string address,string password = "",decimal version = 3,int timeout = 3000,bool uTF8Encoding = true, string tableName = null, params string[] columnsname)
        {
            ...
            try
            {
                ...
                if (tableName != null)
                {
                    ...
                    Execute(CommandText, MiMFa_ExecuteType.ExecuteNonQuery);
                }
        ...
        #region Execute
        public object Execute(string query, MiMFa_ExecuteType executeType, params SQLiteParameter[] parameters)
        {
            if (IsOpen)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        try
                        {
                            switch (executeType)
                            {
                                case MiMFa_ExecuteType.ExecuteScalar:
                                    object s = Command.ExecuteScalar();
                                    transaction.Commit();
                                    ExecuteFinish(this, query, s);
                                    return s;
                                case MiMFa_ExecuteType.ExecuteScalarAsync:
                                    Task<object> ts = Command.ExecuteScalarAsync();
                                    transaction.Commit();
                                    ExecuteFinish(this, query, ts);
                                    return ts;
                                case MiMFa_ExecuteType.ExecuteNonQuery:
                                    IsChanged = true;
                                    int r = Command.ExecuteNonQuery();
                                    transaction.Commit();
                                    ExecuteFinish(this, query, r);
                                    return r;
                                case MiMFa_ExecuteType.ExecuteNonQueryAsync:
                                    IsChanged = true;
                        ...
                        catch (Exception ex)
                        {
                            ...
                            switch (executeType)
                            {
                                ...
                                case MiMFa_ExecuteType.ExecuteNonQueryAsync:
                                    ...
                                    ExecuteFinish(this, query, r);
                                    return r;
                                case MiMFa_ExecuteType.Null:
                                case MiMFa_ExecuteType.ExecuteReader:
                                case MiMFa_ExecuteType.ExecuteReaderAsync:
                                    Adapter.Fill(Result);
                                    DataTable dt = new DataTable();
                                    for (int i = 0; i < Result.Columns.Count; i++)
                                        dt.Columns.Add(Result.Columns[i].ColumnName, typeof(object));
        ...
        public  object Execute(string query, MiMFa_ExecuteType executeType)
        {
            SQLiteParameter[] sqlp = null;
            return Execute(query, executeType, sqlp);
        }
        public  object Execute(string query, MiMFa_ExecuteType executeType, SQLiteParameterCollection parameters)
        {
        ...
        public object Execute(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName,string[] names,params object[] objects)
        {
        ...
        public  object Execute(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, params SQLiteParameter[] parameters)
        {
            return Execute(query, executeType, createParameterName, createColumnName, "", parameters);
        }
        public  object Execute(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, string appendQuery, params SQLiteParameter[] parameters)
        {
            return Execute(CreateQuery(query,createParameterName,  createColumnName,  appendQuery,parameters), executeType, parameters);
        }
        public object Execute(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName,Dictionary<string,object> namesANDobjects)
        {
        ...
        public object Execute(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, object[] objarr)
        {
        ...
        public  object Execute<T>(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, T thisClass)
        {
            return Execute(query, executeType, createParameterName, createColumnName, "", thisClass);
        }
        public  object Execute<T>(string query, MiMFa_ExecuteType executeType, T thisClass)
        {
            return Execute(query, executeType, false, false, "", thisClass);
        }
        public  object Execute<T>(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, string appendQuery, T thisClass)
        {
        ...
        public  object Execute<T>(string query, MiMFa_ExecuteType executeType, Dictionary<string, string> columnDic, bool createParameterName, bool createColumnName, string appendQuery, params T[] classes)
        {
        ...
        public object Execute<T>(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, string appendQuery, params T[] classes)
        {
            Type type = typeof(T);
        ...
        public object Execute<T>(string query, MiMFa_ExecuteType executeType, bool createParameterName, bool createColumnName, params T[] classes)
        {
            return Execute(query, executeType, createParameterName, createColumnName, "", classes);
        }
        public object Execute<T>(string query, MiMFa_ExecuteType executeType, params T[] classes)
        {
            return Execute(query, executeType, false, false, "", classes);
        }
        public object Execute<T>(string query, MiMFa_ExecuteType executeType, DataTable table)
        {
            if (IsOpen)
            {
                Statement.LimitedLoop(() => IsOpen, WaitForSignal, () => { Thread.Sleep(SignalDelay); });
        ...
        char SeprateChar = '*';
        public object MergeQuery(MiMFa_ExecuteType executeType, params KeyValuePair<string, SQLiteParameter[]>[] queries)
        {
            string query = MiMFa_CollectionService.GetAllItems((from v in queries select v.Key).ToArray(), "; ");
            SQLiteParameter[] par = MiMFa_CollectionService.Concat((from v in queries select v.Value).ToArray());
            return Execute(query, executeType, par);
        }
        public KeyValuePair<string, SQLiteParameter[]> CreateQuery<T>(string query, T thisClass)
        {
            ...
            string st = SeprateChar + MiMFa_UnicCode.CreateNewString(20);
            foreach (var item in fi) try
                {
                    var obj = item.GetValue(thisClass);
                    if (obj != null)
                        if (obj is System.Drawing.Image)
                            obj = MiMFa_Convert.ToByteArray((System.Drawing.Image)obj);
                        else if (!MiMFa_GetDetail.IsNumber(obj)
                            && !MiMFa_GetDetail.IsText(obj))
                            obj = MiMFa_IOService.Serialize(obj);
        ...
        {
            Type type = typeof(T);
            string columns = MiMFa_CollectionService.GetAllKeysItem(columnDic, " , ");
            ...
            {
                 brk = false;
                string st = string.Join("", SeprateChar , MiMFa_UnicCode.CreateNewInciment() , suffix);
                int i = 0;
                ...
                foreach (var item in columnDic)
                {
                    ...
                    values.Add(val);
                    object o = null;
                    PropertyInfo[] pi = MiMFa_GetDetail.GetProperties(item.Key, type, true);
                    if (pi.Length > 0)
                    {
                    ...
                    {
                        FieldInfo[] fi = MiMFa_GetDetail.GetFields(item.Key, type, true);
        ...
        {
            Execute("DROP TABLE IF EXISTS " + tableName, MiMFa_ExecuteType.ExecuteNonQuery);
        }
        public object Create(string tableName, Dictionary<string, string> columnDic, string attachment = "")
        {
            return Create(tableName, MiMFa_CollectionService.GetAllItems(columnDic," "," , "), attachment);
        }
        public  object Create(string tableName, string columns, string attachment = "")
        {
            ...
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery);
        }
        ...
        {
            return Convert.ToDouble(Execute("SELECT Count(*) FROM " + tableName, MiMFa_ExecuteType.ExecuteScalar));
        }
        ...
        public DataTable Select(string query)
        {
            ...
            {
                return (DataTable)Execute(query, MiMFa_ExecuteType.ExecuteReader);
            }
        }
        ...
        public object Insert(string tableName, DataTable dt, string condition = "")
        {
            try
            {
                ...
                return Execute(q.Key, MiMFa_ExecuteType.ExecuteNonQuery, q.Value);
            }
        ...
        {
            string query = string.Join("", @"INSERT OR REPLACE INTO " , tableName, " ");
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery, true,false, dr.ItemArray);
        }
        public object Insert(string tableName, object[] oa)
        {
            string query = string.Join("", @"INSERT OR REPLACE INTO " , tableName , " ");
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery, true,false, oa);
        }
        public object Insert<T>(string tableName, Dictionary<string, string> columnDic, string condition,params T[] obj)
        {
            return Execute(@"INSERT OR REPLACE INTO " + tableName, MiMFa_ExecuteType.ExecuteNonQuery, columnDic, true,true,condition,obj);
        }
        public object Insert(string tableName, Dictionary<string, string> columnDic, object obj, string condition = "")
        {
            ...
            {
                object o = null;
                PropertyInfo[] pi = MiMFa_GetDetail.GetProperties(obj, item.Key, true);
                if (pi.Length > 0) o = pi.First().GetValue(obj);
                else {
                    FieldInfo[] fi = MiMFa_GetDetail.GetFields(obj, item.Key, true);
                    if (fi.Length > 0) o = fi.First().GetValue(obj);
                }
                sodic.Add(item.Key, TYPE.Set(o,item.Value));
            }
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery,true,true, sodic);
        }
        public object Insert(string tableName, string columns, string values, string condition = "",SQLiteParameter[] parameters = null)
        {
            ...
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery,false, false, parameters);
        }
        public object Insert(string tableName,string condition = "", SQLiteParameter[] parameters = null)
        {
            string query = @"
            INSERT OR REPLACE INTO " + tableName ;
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery,true,true,parameters);
        }
        ...
        public object Insert(string tableName, string columns, string values, string condition = "",SQLiteParameterCollection parameters = null)
        {
            string query = string.Join("", @"
            INSERT OR REPLACE INTO " , tableName ," (" , columns , ") VALUES(" , values , ") " , ((condition != null) ? condition:"") ,";");
            return Execute(query, MiMFa_ExecuteType.ExecuteNonQuery,parameters);
        }
        ...
        {
            return Execute(string.Join("", @"SELECT " , columnName , " FROM " , tableName , " " , condition , " ;"), MiMFa_ExecuteType.ExecuteReader);
        }
        ...
        {
            DataTable dt = (DataTable)Execute(string.Join("", @"SELECT * FROM " , tableName , " WHERE FromTable = '" , FromTable , "' AND FromColumn = '" , fromColumn , "' AND FromRecord = '" , FromRecord , "' ; "), MiMFa_ExecuteType.ExecuteReader);
            if (dt.Rows.Count > 0)
        ...
        {
            return Execute(string.Join("", @"INSERT OR REPLACE INTO " , tableName , " VALUES ('" , FromTable , "','" , fromColumn , "','" , FromRecord , "', @Value,'", ((value != null)?value.GetType().Name : "Null" ),"') " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery, new SQLiteParameter("@Value", TYPE.Set(value,TYPE.BLOB)));
        }
        ...
        public Dictionary<string, T> GetDic<T>(string tableName, string condition = "")
        {
            DataTable dt = (DataTable)Execute(
               string.Join("", @"SELECT * FROM " ,
                tableName , " " , condition , " ;"), MiMFa_ExecuteType.ExecuteReader);
        ...
        public  object[] GetKeys(string tableName, string value, string condition = "", bool blobValue = false)
        {
            DataTable dt = (DataTable)Execute(string.Join("", @"SELECT * FROM " ,
                tableName , " WHERE Value = '" , value ,"' " , condition , " ;"), MiMFa_ExecuteType.ExecuteReader);
        ...
        {
            DataTable dt = (DataTable)Execute(string.Join("", @"SELECT * FROM " ,
                tableName , " WHERE Sign Like '" , key, "' " , condition, " ;"), MiMFa_ExecuteType.ExecuteReader);
            if (dt.Rows.Count > 0)
                if (blobValue) return MiMFa_IOService.Deserialize((byte[])dt.Rows[0]["Value"]);
        ...
        public object InsertValues(string tableName, Dictionary<string, object> columnValues, string condition)
        {
            ...
            foreach (var item in columnValues)
            {
                ...
                object value = MiMFa_IOService.Serialize(item.Value);
                sqls.Add(new SQLiteParameter("@Value" + i, value));
                i++;
            }
            return Execute(string.Join("", @"INSERT OR REPLACE INTO " , tableName ," ",keys , " VALUES ", string.Join(",", values), " " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery, sqls.ToArray());
        }
        public object InsertValue(string tableName, string key, object value, string condition, bool blobValue = false)
        {
            if (blobValue)
                value = MiMFa_IOService.Serialize(value);
            return Execute(string.Join("", @"INSERT OR REPLACE INTO " , tableName, " VALUES ('" , key , "', @Value) " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery, new SQLiteParameter("@Value", value));
        }
        public  object GetValue(string tableName, string key, string condition, bool blobValue = false)
        {
            DataTable dt = (DataTable)Execute(string.Join("", @"SELECT * FROM ", tableName , " WHERE Sign = '", key , "' ", condition , " ;"), MiMFa_ExecuteType.ExecuteReader);
            if (dt.Rows.Count > 0)
                if (blobValue && dt.Rows[0]["Value"] is byte[]) return MiMFa_IOService.Deserialize((byte[])dt.Rows[0]["Value"]);
        ...
        public object SetValues(string tableName, Dictionary<string, string> columnValues, string condition)
        {
            ...
            return Execute(string.Join("", @"UPDATE " , tableName ,
                " SET ",string.Join(" , ", sets)," " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery);
        }
        public object SetValues(string tableName, Dictionary<string, object> columnValues, string condition)
        {
            ...
            return Execute(string.Join("", @"UPDATE " , tableName , " SET ", string.Join(" , ", sets)," " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery, sqls.ToArray());
        }
        public object SetValue(string tableName, KeyValuePair<string, string> columnValue, string condition, bool blobValue = false)
        {
            ...
            if (blobValue)
                value = MiMFa_IOService.Serialize(value);
            return Execute(string.Join("", @"UPDATE " , tableName, " SET [" , columnValue.Key , "] = @Value " , condition , " ;"), MiMFa_ExecuteType.ExecuteNonQuery, new SQLiteParameter("@Value", value));
        }
        #endregion
    ...
    #region Other
    [Serializable]
    public class MiMFa_SQLite_Types : MiMFa_DataBase_ITypes
    {
                ...
                case "BLOB":
                    return (obj == null || obj == o) ? new byte[]{ } : 
                        (obj.GetType().IsAssignableFrom(typeof(Image))|| obj.GetType().IsAssignableFrom(typeof(Bitmap))) ?
                            MiMFa_Convert.ToByteArray((Bitmap)obj) :
                            MiMFa_IOService.Serialize(obj);
                case "IMAGE":
                    return (obj == null) ? null : MiMFa_Convert.ToByteArray((Image)obj);
                case "BITMAP":
                    return (obj == null) ? null : MiMFa_Convert.ToByteArray((Bitmap)obj);
                default:
                    return obj;
            }
        }
        public object Get(object obj, string type = null)
        {
            ...
            try
            {
                switch (type)
                {
                    ...
                    case "BLOB":
                        if (obj == null) return null;
                        else try { return MiMFa_IOService.Deserialize((byte[])obj); }
                            catch
                            {
                                return MiMFa_Convert.ToImage((byte[])obj);
                            }
                    case "BYTE[]":
                        if (obj == null) return null;
                        else try { return MiMFa_IOService.Deserialize((byte[])obj); }
                    ...
                    case "IMAGE":
                        return (obj == null) ? null : MiMFa_Convert.ToImage((byte[])obj);
                    case "BITMAP":
                        return (obj == null) ? null : MiMFa_Convert.ToImage((byte[])obj);
                    default:
                        return obj;
