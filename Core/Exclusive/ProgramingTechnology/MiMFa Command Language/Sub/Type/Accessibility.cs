...
namespace MiMFa_Framework.Exclusive.ProgramingTechnology.CommandLanguage
{
    ...
    {
        public MiMFa_ProgrammingAccessibility Status = MiMFa_ProgrammingAccessibility.Internal;
        public long ID = MiMFa_UnicCode.CreateNewLong();
        ...
        public static bool IsAccess(Accessibility environment, Accessibility node)
        {
            ...
            if (environment == null)
                if (node.Status == MiMFa_ProgrammingAccessibility.Public 
                    || node.Status == MiMFa_ProgrammingAccessibility.Internal) return true;
                else return false;
            if (node.Status == MiMFa_ProgrammingAccessibility.Public) return true;
